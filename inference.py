...
parser.add_argument('--inputvideo', dest='inputvideo', help='input video', type=str, default='videos/篮球2.mp4')
parser.add_argument('--outputvideo', dest='outputvideo', help='output video', type=str, default='videos/result_videos/篮球2.mp4')
parser.add_argument('--outputimagesdir', dest='outputimagesdir', help='output images', type=str, default='videos/篮球')
parser.add_argument('--samplefrequency', dest='samplefrequency', help='video samplefrequency', type=int, default=4)
parser.add_argument('--model', dest='model', help='model', type=str, default='R3D')
parser.add_argument('--weightspath', dest='weightspath', help='input directory for modelweights', type=str, default='checkpoints/UCF-101/C3D/models/C3D-ucf101_epoch-49.pth.tar')
args = parser.parse_args()

torch.backends.cudnn.benchmark = True

def CenterCrop(frame, size):#计算视频中心进行偏移裁剪
...
def center_crop(frame):
    frame = frame[8:120, 30:142, :]#指定偏移量进行裁剪
    return np.array(frame).astype(np.uint8)
...
def main():
    ...
    with open('D:/project/3DCNN/3DCNN/dataloaders/ucf_labels.txt', 'r') as f:
    ...
    clip = [] ##缓冲图像序列
    frequency = args.samplefrequency ##采样频率指间隔多少帧采样一张图片
    i = 0
    while retaining:
        ...
        tmp = tmp_ - np.array([[[90.0, 98.0, 102.0]]])#减去均值
        clip.append(tmp)
        ...
        if len(clip) == 16:
            ...
            probs = torch.nn.Softmax(dim=1)(outputs)#概率计算
            label = torch.max(probs, 1)[1].detach().cpu().numpy()[0]#获取概率最大的标签
            ...
            clip.pop(0)#丢弃掉最前面的1帧，使得能够推理以后的帧
